...
import {
  User, Building2, Users, Pencil, Trash2, KeyRound,
  Save, X, AlertTriangle, Eye, EyeOff, CheckCircle
} from 'lucide-react'
...
export default function RecruiterProfile() {
  ...
  const [form, setForm]   = useState({ recruiter_name: '', company_id: '' })
  const [pwForm, setPwForm] = useState({ current_password: '', new_password: '', confirm_password: '' })
  const [deletePassword, setDeletePassword] = useState('')

  const { data: profile, isLoading } = useQuery({
    queryKey: ['recruiter-profile'],
    queryFn: () => fetch('/api/recruiters/me').then(r => r.json()).then(d => d.data),
    onSuccess: (data: any) => {
      if (data) setForm({ recruiter_name: data.recruiter_name ?? '', company_id: String(data.company_id ?? '') })
    },
  } as any)
  ...
  function handleSave() {
    if (!form.recruiter_name.trim()) { add('Name cannot be empty', 'error'); return }
    updateMutation.mutate({ recruiter_name: form.recruiter_name, company_id: form.company_id })
  }
  ...
  function handleCancelEdit() {
    ...
    setShowPwSection(false)
    setPwForm({ current_password: '', new_password: '', confirm_password: '' })
    if (profile) setForm({ recruiter_name: profile.recruiter_name ?? '', company_id: String(profile.company_id ?? '') })
  }

  if (isLoading) return (
    <DashboardLayout>
      <div className="max-w-xl mx-auto space-y-4">
        {Array(3).fill(0).map((_, i) => <div key={i} className="glass rounded-2xl h-24 border border-white/[0.06] animate-pulse" />)}
      </div>
    </DashboardLayout>
  )
    ...
    <DashboardLayout>
      <div className="max-w-xl mx-auto space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
          className="glass rounded-2xl p-6 border border-violet-500/20 bg-gradient-to-r from-violet-500/5 to-pink-500/5 flex items-center gap-5"
        >
          <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-violet-500 to-purple-500 flex items-center justify-center text-white text-2xl font-bold shrink-0">
            {profile?.recruiter_name?.charAt(0)}
          </div>
          <div>
            <h2 className="font-display text-xl font-bold text-foreground">{profile?.recruiter_name}</h2>
            <p className="text-sm text-muted-foreground">{profile?.company?.company_name} — {profile?.company?.location}</p>
            <p className="text-xs text-violet-400 font-mono mt-1">recruiter_id: {profile?.recruiter_id}</p>
          </div>
        </motion.div>
        ...
        <AnimatePresence mode="wait">
          {!editMode ? (
            <motion.div key="view" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
              {[
                { icon: User,     label: 'Recruiter Name', value: profile?.recruiter_name,    color: 'text-violet-400' },
                { icon: Building2,label: 'Company',        value: profile?.company?.company_name, color: 'text-blue-400' },
                { icon: Building2,label: 'Location',       value: profile?.company?.location,  color: 'text-cyan-400' },
                { icon: Users,    label: 'Interviews Conducted', value: `${profile?.interviews?.length ?? 0} interview(s)`, color: 'text-amber-400' },
              ].map((item, i) => (
            ...
            <motion.div key="edit" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
              ...
              <div className="glass rounded-2xl p-5 border border-violet-500/20 space-y-4">
                <h2 className="font-display font-semibold text-sm text-foreground flex items-center gap-2">
                  <Pencil size={14} className="text-violet-400" /> Edit Details
                </h2>
                <div>
                  <label className="text-xs font-medium text-muted-foreground mb-1.5 block">Recruiter Name</label>
                  <input className="input-field" value={form.recruiter_name}
                    onChange={e => setForm(f => ({ ...f, recruiter_name: e.target.value }))} placeholder="Full name" />
                </div>
                <div>
                ...
                <motion.button onClick={handleSave} disabled={updateMutation.isPending} whileTap={{ scale: 0.98 }}
              ...
              <div className="glass rounded-2xl border border-white/[0.06] overflow-hidden">
                ...
                >
                  ...
                  <motion.div animate={{ rotate: showPwSection ? 45 : 0 }} transition={{ duration: 0.2 }}>
                      ...
                      {[
                        ...
                        <div key={f.key}>
                          ...
                          <div className="relative">
                            <input type={f.show ? 'text' : 'password'} className="input-field pr-10"
                              placeholder="••••••••"
            ...
            >
              ...
              >
                ...
                <p className="text-sm text-muted-foreground text-center mb-5">
                  This permanently deletes your recruiter account and <strong className="text-foreground">all conducted interviews</strong>. Cannot be undone.
                </p>
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2 pt-1">
                    <button onClick={() => { setShowDeleteModal(false); setDeletePassword('') }} className="btn-secondary flex-1 justify-center">Cancel</button>
                    <button
                      disabled={!deletePassword || deleteMutation.isPending}
                      onClick={() => deleteMutation.mutate()}
                      className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold bg-red-500 text-white hover:bg-red-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                    >
